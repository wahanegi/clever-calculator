ActiveAdmin.register Quote do
  ...
  filter :user, as: :select, collection: proc { User.joins(:quotes).distinct.order(:email).map { |u| ["#{u.email} (#{u.name})", u.id] } }
  ...
  form do |f|
    f.inputs do
      f.input :customer, as: :select, collection: Customer.pluck(:first_name, :last_name, :id).map { |c| ["#{c[0]} #{c[1]}", c[2]] }
