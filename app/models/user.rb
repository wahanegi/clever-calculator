class User < ApplicationRecord
  ...
  validates :email, presence: true, uniqueness: true, format: { with: EMAIL_FORMAT, message: "must be a valid email format" }
  validates :password, length: { minimum: 8 }, format: { with: PASSWORD_SYMBOL_FORMAT, message: "must contain at least one symbol" }
