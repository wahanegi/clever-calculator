...
gem "kamal", '~> 2.6.1', require: false                             # Deploy this application anywhere as a Docker container [https://kamal-deploy.org]
...
gem "solid_cable", '~> 3.0.10'                                      # Use the database-backed adapters for Action Cable
...
group :production do
  gem "aws-sdk-s3", '~> 1.189.1'                   # AWS SDK for Ruby
end
